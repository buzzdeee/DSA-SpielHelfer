...
#define MARGIN 18 // Example margin
...
- (void)printDocument:(id)sender
{
  ...
  MyMultiPageView *printView = [[MyMultiPageView alloc] initWithFrame:NSMakeRect(0, 0, PAGE_WIDTH, PAGE_HEIGHT * pages)];
...
- (NSRect)rectForPage:(NSInteger)page
{
  ...
  NSRect pageRect = NSMakeRect(-MARGIN, (page - 1) * pageHeight, self.bounds.size.width + MARGIN, pageHeight);
...
- (CGFloat) drawBasicCharacterInfoTableStartingAtY:(CGFloat)y {
    ...
    CGFloat sixthColumnWidth = tableWidth * 0.26; // 20% of total width for even columns (6th column)
...
- (void)drawPageSeparator:(NSUInteger)pageNumber
{
  ...
  NSString *pageText = [NSString stringWithFormat:@"%@ Seite %lu", [self.model name], (unsigned long)pageNumber];
    
  // Calculate the position to draw the text
  NSSize textSize = [pageText sizeWithAttributes:@{NSFontAttributeName: [NSFont systemFontOfSize:8]}];
  NSPoint textPosition = NSMakePoint(self.bounds.size.width - textSize.width, // Adjusted for margin
                                     [self paperHeightForPage] * pageNumber - textSize.height); // Adjusted for vertical positioning
    
  NSLog(@"%@, %@, %lu", pageText, NSStringFromPoint(textPosition), (unsigned long)self.bounds.size.width);                                                                              
  [pageText drawAtPoint:textPosition
