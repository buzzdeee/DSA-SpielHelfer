...
- (NSDictionary *) levelUpBaseEnergies
{
  ...
  NSNumber *result = [NSNumber numberWithInteger: [[Utils rollDice: @"1W6"] integerValue]];
 
  if ([self.astralEnergy integerValue] + [result integerValue] > 30) // AstralEnergy can't go above 30
    {
      NSInteger diff = 30 - [self.astralEnergy integerValue];
      self.lifePoints = [NSNumber numberWithInteger: [self.lifePoints integerValue] + diff];
      self.tempDeltaLpAe = [NSNumber numberWithInteger: [result integerValue] - diff];
      // we have to ask the user how to distribute these
      [resultDict setObject: self.tempDeltaLpAe forKey: @"deltaLpAe"];  
      [resultDict setObject: [NSNumber numberWithInteger: diff] forKey: @"deltaLifePoints"];
    }
  else
    {
