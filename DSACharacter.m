...
+ (DSACharacter *)characterWithModelID:(NSUUID *)modelID {
    @synchronized(characterRegistry) {
        ...
        if (character) {
            //NSLog(@"DSACharacter characterWithModelID: Found matching modelID: %@", [modelID UUIDString]);
        } else {
            NSLog(@"DSACharacter characterWithModelID: Character with modelID: %@ not found", [modelID UUIDString]);
        }
        return character;
    }
}
          ...
          for (NSString *damageType in oneTimeDamage)
            {
              ...
              if ([damageType isEqualToString:@"Eigenschaften"])
                  ...
                  for (NSString *atpa in atpaChanges)
                    {
                      ...
                      if (![modifierNumber isKindOfClass:[NSNumber class]])
                        {
                           NSLog(@"DSACharacter applyOneTimeDamage: Invalid Modifier value for atpa '%@': %@ ABORTING", atpa, modifierNumber);
                           abort();
                           continue;
                        }
                      ...
                      if ([atpa isEqualToString: @"AT"])
                        ...
                        {
                          NSLog(@"DSACharacter applyOneTimeDamage: unknown AttackeParade: %@ ABORTING", atpa);
                          abort();
                        }
                    }
                }
                ...
                {
                  NSLog(@"DSACharacter applyOneTimeDamage: unsupported damageType: %@ ABORTING", damageType);
                  abort();
                }
            }
       }
                                  ...
                                  revert: (BOOL) shouldRevert
{
   ...
   if (![modifierNumber isKindOfClass:[NSNumber class]])
     {
        NSLog(@"Invalid Modifier value for trait '%@': %@ ABORTING", trait, modifierNumber);
        abort();
        return;
     }
   ...
   NSLog(@"DSACharacter applyOneTimeTraitChangesFromDict: unknown trait: %@ ABORTING", trait);
   abort();
}                                 
...
-(void) applyRegularDamageForCharacterEffect: (DSACharacterEffect *) characterEffect atDate: (DSAAventurianDate *) currentDate
{
  ...
  for (NSString *who in [recurringDamageDict allKeys])
          ...
          for (NSString *damageType in [recurringDamage allKeys])
           {
              ...
              if ([damageType isEqualToString: @"Damage"])
                  ...
                  for (NSString *damage in [damageDict allKeys])
                    {
                      if ([damage isEqualToString: @"SP"])
                        ...
                        {
                          NSLog(@"DSACharacter applyRegularDamageForCharacterEffect: inner if/else don't know how to handle damage: %@ ABORTING", damage);
                          abort();
                        }
                    }
                }
                ...
                {
                   NSLog(@"DSACharacter applyRegularDamageForCharacterEffect: outer if/else don't know how to handle damageType: %@ ABORTING", damageType);
                   abort();
                }
            }
        }
   ...
   if ([recurringIntervalDescriptor isEqualToString: @"KR"])  // handle seconds?
     ...
     {
       NSLog(@"DSACharacter applyRegularDamageForCharacterEffect: do not know how to handle recurringIntervalDescriptor: %@ ABORTING", recurringIntervalDescriptor);
       abort();
     }
}
...
- (BOOL) autoConsumeFood
{
    DSAAdventure *adventure = [DSAAdventureManager sharedManager].currentAdventure;
    NSString *foodName = @"Proviant, Tagesration";
    DSAInventoryManager *inventoryManager = [DSAInventoryManager sharedManager];
    DSAObject *food = [inventoryManager findObjectWithName: foodName inInventory: self.inventory];
    BOOL consumed = NO;
    if (food)
      {
        consumed = [self consumeItem: food
                              atDate: adventure.gameClock.currentDate];
        if (consumed)
          {
            DSASlot *inventorySlot = [inventoryManager findSlotInModel: self
                                               withInventoryIdentifier: @"inventory"
                                              containingObjectWithName: foodName];
            [inventoryManager handleItemInSourceSlot: inventorySlot
                                       ofSourceModel: self];
          }
      }
    return consumed;
...
- (void) updateStateThirstWithValue: (NSNumber*) value
{
    ...
    else if (newLevel < currentLevel)
    {
        if (newLevel == 0)
        {
          ...
          {
            consumed = [self autoConsumeDrink];
          }        
        else if (newLevel < 0.1)
        {
            consumed = [self autoConsumeDrink];
            // ZUSÄTZLICHE PRÜFUNG: Nur Benachrichtigung senden, wenn sie noch NICHT gesendet wurde
            if (!consumed && !self.thirstWarningSent) // <--- NEU
            {
...
- (BOOL) autoConsumeDrink
{
    BOOL consumed = NO;
    DSAAdventure *adventure = [DSAAdventureManager sharedManager].currentAdventure;
    NSString *subCategory = @"Wasserbehälter";
    DSAInventoryManager *inventoryManager = [DSAInventoryManager sharedManager];
    NSArray <DSAObject *> *containers = [inventoryManager findItemsBySubCategory: subCategory 
                                                                         inModel: self];
    if (containers && [containers count] > 0)
      {
        for (DSAObject *container in containers)
          {
            DSAObjectContainer *containerObject;
            if (![container isKindOfClass: [DSAObjectContainer class]])
              {
                continue;
              }
            else
              {
                containerObject = (DSAObjectContainer *)container;
              }  
            if (![containerObject isEmpty])
              {
                consumed = [self consumeItem: containerObject
                                      atDate: adventure.gameClock.currentDate];
                if (consumed)
                  {
                    break;
